...
cm.merge = function(o1, o2){
	...
	cm.foreach(o2, function(key, item){
		if(item != null){
			try{
...
cm.clone = function(obj, type){
	if(!obj){
		...
		var o = {};
		cm.foreach(obj, function(key, item){
			if(typeof item == 'object'){
				o[key] = cm.clone(item);
			}else{
				o[key] = item;
...
cm.onScrollStart = function(node, handler, delay){
	var worked = false,
		scrollEnd = function(){
			worked = false;
		},
		helper = _.debounce(scrollEnd, delay || 300),
...
cm.onScrollEnd = function(node, handler, delay){
	var helper = _.debounce(handler, delay || 300);
...
cm.dateFormat = function(date, format, langs){
	...
	cm.foreach(formats(date), function(key, item){
...
cm.ajax = cm.altReq = function(){
	...
	var httpRequestObject = o.httpRequestObject? o.httpRequestObject : cm.createXmlHttpRequestObject();
...
cm.reloadHashData = function(){
	var hash = '#';
	cm.foreach(window.userRequest, function(key, item){
		hash += key + '=' + item;
	});
